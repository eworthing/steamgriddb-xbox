# Contest Refactor — SteamGridDB Xbox Game Bar widget
...
- **Test command:** **none — this repository has no test project.** Verified: no `*.csproj`
  ...
  which outranks this skill on the instruction-authority ladder. Consequence for the loop: the
  build command below is the *only* ground-truth oracle, so Step 3's regression check is a
  type-check, not a behavioural one. `test_scope: "full"`, `test_filter: null`.
- **Build command** (the ground-truth gate; `msbuild` is not on PATH, resolved via `vswhere`
  ...
  Verified green at `92c598a` in **4 seconds** (exit 0). Re-verified green after this loop's
  refactor (below). Well under the 5-minute warning threshold, so no `--test-filter` guidance
  applies.
- **ADRs found:** none (`docs/adr/` absent).
- **Domain terms (CONTEXT.md):** none (`CONTEXT.md`, `CLAUDE.md`, `AGENTS.md` all absent).
- **Prior audit docs** (tier-4 payload evidence — adopt-or-falsify performed below, *after* the
  independent scorecard was drafted):
  - `CODE-REVIEW.md` (324 lines, 2026-08-03) — whole-repository review against `dfa22fb`. The
    document's own `## Status — all fixed` header explicitly marks every one of its 15 numbered
    findings resolved, so none are "open claims" under the adopt-or-falsify rule. Spot-checked
    three against current source rather than trusting the self-report blindly: #8
    (`DecodePixelWidth` cap) confirmed present (`PrimaryWidget.xaml.cs:51,328`, constant
    `thumbnailDecodePixelWidth = 160`); #10 (`GridMetadata` recomputed 3x per grid) confirmed
    fixed — `RankedGrid`'s constructor now computes it once (moved this loop to
    `Services/Artwork/ArtworkRanker.cs:167-183`); #13 (`GridStylePriority` duplicated inline)
    confirmed fixed — `GetTitleBearingGridsAsync` calls the shared method
    (`PrimaryWidget.xaml.cs:1698`, now `ArtworkRanker.GridStylePriority`). No falsifications
    found; no action taken.
  - `ARTWORK-SELECTION.md` (593 lines, 2026-08-03) — an artwork-*ranking-quality* research log
    (grading verdicts, tuning history), not an architecture document; out of this skill's scope
    by definition (`SKILL.md`: "not correctness, security, performance, or feature-completeness").
    Its one item marked outstanding (§4.8, "corner gate at 9% reach") is a product-behavior
    proposal the document's own analysis already deprioritizes ("no evidence calls for it") —
    not a live claim against any scored architecture dimension. No adopt-or-falsify action taken.
- **Selected lens:** Generic (`lens-generic.md`). C#/.NET/UWP matches no row in the lens
  registry, so it takes the "anything else" fallback. **Known gap, stated here since it did not
  bite this loop:** the generic lens covers Rust/Go/Python/Node/JVM idioms and knows nothing
  about the UWP specifics this codebase actually lives on — `IAsyncOperation`/`IAsyncAction`
  boundaries, `SoftwareBitmap` native-memory disposal, app-container path virtualisation, and
  `Windows.Data.Json` null semantics. This loop's findings did not depend on any of those four;
  future loops should not read that as clearance.
...
- **Churn top-20** (`git log --since="6 months ago"`, `.cs` under the source root) — the whole
  history is inside the window, so this is total edit count, not a recent-activity slice:

  | edits | path |
  |---|---|
  | 21 | `SteamGridDB.Xbox/PrimaryWidget.xaml.cs` |
  | 8 | `SteamGridDB.Xbox/Services/SteamGridDB/SteamGridDbClient.cs` |
  | 4 | `SteamGridDB.Xbox/Models/GameEntry.cs` |
  | 3 | `SteamGridDB.Xbox/Models/GridImageItem.cs` |
  | 2 | `SteamGridDB.Xbox/Services/Stores/EpicLibrary.cs` |
  | 2 | `SteamGridDB.Xbox/Services/SteamGridDB/Models/SteamGridDbGrid.cs` |
  | 2 | `SteamGridDB.Xbox/Services/Artwork/TileImage.cs` |
  | 2 | `SteamGridDB.Xbox/Services/Artwork/FixLog.cs` |
  | 2 | `SteamGridDB.Xbox/Services/Artwork/ArtworkSignature.cs` |
  | 2 | `SteamGridDB.Xbox/Models/GamePlatform.cs` |
  | 1 | `SteamGridDB.Xbox/Services/SteamGridDB/Models/SteamGridDbGame.cs` |
  | 1 | `SteamGridDB.Xbox/Services/SteamGridDB/ArtworkSource.cs` |
  | 1 | `SteamGridDB.Xbox/Services/JsonRead.cs` |
  | 1 | `SteamGridDB.Xbox/Services/Artwork/AppliedArtworkStore.cs` |
  | 1 | `SteamGridDB.Xbox/Properties/AssemblyInfo.cs` |

  `PrimaryWidget.xaml.cs` took 2.6x the edits of the next file and was 6.6x its size at loop
  start — the mandatory churn-vs-abstraction cross-check (`method.md` Step 3) applied to it
  directly; see Finding F1.
- **Preflight gate:** `preflight.py SteamGridDB.Xbox --test-cmd <build command> --base-ref master`
  → exit 0, "scope dir, test command, and base ref all resolve."
...
- **Structurally necessary:** Extracting the artwork-ranking classification cluster (F1's first
  slice) into `Services/Artwork/ArtworkRanker.cs` — passes the deletion test in reverse (the
  logic is real and load-bearing; it just did not belong on the UI class) and is a pure move with
  zero new abstraction.
- **New seam justified:** false — no new Seam was created; the cluster merged into the existing
  `Services/Artwork/` module.
- **Helpful simplification:** Consolidating the four store-name caches and the artwork-ranking
  constants under `Services/` once F1's remaining slices land.
- **Should NOT be done:** Introduce an `IArtworkRanker` interface, a Coordinator, or a DI
  container for this move — there is exactly one production caller (`PrimaryWidget`) and no
  second implementation is proven necessary. That would be architecture costume layered on what
  is otherwise a one-line static-class-reference fix.
- **Tests after fix:** No test project exists (standing user instruction); the build (MSBuild
  compile) is the only regression oracle this loop, consistent with Discovery's stated
  `test_command_absent_reason`.

## Improvement Backlog

1. **Fix AppliedArtworkStore.GetAsync's unguarded read against UpdateAsync's guarded write**
   (F2) — structural, needed for winning.
   - Why it matters: closes a real concurrency hazard in a Module every artwork-write path
     depends on.
   - Score impact: State management +0.5, Concurrency +0.5 once verified.
...
3. **Continue the PrimaryWidget.xaml.cs break-up (F1, next slice): store-name resolution**
   (GOG/Epic/Ubisoft fetch methods + their four caches) into `Services/Stores/` alongside the
   existing `EpicLibrary.cs` — structural, needed for winning.
   - Why it matters: removes the largest remaining slice of F1; `EpicLibrary.cs` already
     establishes the precedent this fetch logic should follow.
...
- **Candidate Module:** An `AsyncLazy`-style cache helper for `AppliedArtworkStore` +
  ...
  - Source friction proven: F4 — identical double-checked-locking skeleton hand-copied between
    `AppliedArtworkStore.cs:84-136` and `EpicLibrary.cs:67-89`.
  ...
  - Test surface after change: no test project (standing instruction); build-verified only, same
    as today.
  - Smallest first step: write the helper against `AppliedArtworkStore` first (it also has the F2
    race to fix at the same time), then `EpicLibrary` once the shape is proven.
...
1. **Pattern:** Churn concentrates where concerns are merged.
   - How to recognize: one file dominates the six-month edit count and is several times larger
     than everything else; commits touching it describe unrelated changes (a ranking tweak, a UI
     fix, a parsing fix) in the same file.
   - Smallest coding rule: when a file's edit count and size both dominate the repo, treat that
     as a leaky-seam signal before writing the next feature into it — extract the concern that
     changed, don't add to the pile.
   - Stack example: C#/UWP — a `Page` code-behind file is a natural dumping ground because event
     handlers must live there; pure business logic (ranking, parsing, name resolution) does not
     have to — move it to a plain static class or service the Page calls into.

2. **Pattern:** A lock that guards the writer but not the reader is not a lock.
   - How to recognize: a `SemaphoreSlim` (or `lock`) wraps every write to a shared mutable
     collection, but at least one read path skips it because "it's just a read."
   - Smallest coding rule: every access path to lock-protected shared mutable state takes the
     same lock, reads included — a `Dictionary` read racing a `Dictionary` write is exactly as
     unsafe as two racing writes.
   - Stack example: C# — `Dictionary<K,V>` is not thread-safe for any concurrent read+write mix;
     either guard every access uniformly or switch to `ConcurrentDictionary<K,V>`.

3. **Pattern:** Sequential `await` in a per-item loop hides its own cost until the collection
   grows.
   - How to recognize: a `foreach` loop with an `await` inside it, over items whose network/IO
     calls don't depend on each other, and no `Task.WhenAll` / bounded-parallel pattern anywhere
     nearby.
   - Smallest coding rule: before parallelizing, check what the loop body writes to outside
     itself (caches, counters) — if those aren't thread-safe, fix that first or the
     "optimization" introduces the very race the sequential version was accidentally protecting
     against.
   - Stack example: C# — `Task.WhenAll` over a bounded batch (via `SemaphoreSlim`) turns N
     sequential round-trips into N/batch-size overlapped ones, but only after any shared
     `Dictionary` the bodies write to becomes a `ConcurrentDictionary`.

**Scorecard humility check** (Q9): three specific claims I am least confident about —
1. `architecture_quality` at 5.0 rather than 4.0 or 6.0 — uncertainty because the surrounding
   `Services/*` modules are genuinely well-structured (`SteamGridDbClient.cs`,
   `ArtworkSource.cs`), so the score averages a bimodal codebase rather than a uniform read; a
   reviewer weighting the god-class more heavily could reasonably land at 4.0.
2. The `AppliedArtworkStore.GetAsync` race (F2, `AppliedArtworkStore.cs:38-48`) — uncertainty
   because I have not observed it fire; the app's own call-site discipline
   (`IsLibraryOperationBlocking` gates most paths that reach `SetAsync` while the picker's
   `GetAsync` path is also reachable) may make it practically unreachable today, which would move
   this closer to Noticeable than Serious.
3. `test_strategy` at 3.0 — uncertainty because the standing no-test-project instruction makes
   this a policy floor rather than a purely structural read; a codebase this size with a real
   test suite might otherwise score respectably on the other 8 dimensions, so 3.0 measures policy
   compliance as much as architecture.
