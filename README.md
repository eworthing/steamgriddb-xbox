...
- **Fix my library** - downloads the best artwork for every third-party game that has a SteamGridDB match. The dialog offers two runs: *Fix new games* only touches games that have not been customised yet, while *Re-fix all games* also re-downloads artwork for games customised earlier, replacing their current images. Games with no match are skipped by both, and so are the Xbox app's own games: their tiles are already the Store's own box art, which is the publisher's official cover rather than the generic image the app derives for a third-party entry. The summary says how many were left alone. To change one of them, use **Change artwork** on its row.
...
- First-party games are found by what they install: the `MicrosoftGame.config` a modern title writes beside its executable, or - for the older Store titles that predate that file, such as Microsoft Mahjong, Microsoft Solitaire Collection and the Halo Spartan games - the Xbox Live protocol handler its package manifest registers. A Store game that has neither is not found.
