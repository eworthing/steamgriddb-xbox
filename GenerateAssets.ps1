...
param(
    ...
    [string]$OutputDir = "SteamGridDB.Xbox\Assets"
)
...
try {
    $magickVersion = Get-ImageMagickVersion
...
# Resolve relative to this script's location, not the caller's working directory - a relative
# default assumed a specific cwd and resolved nowhere useful run from the repo root
...
foreach ($asset in $assets) {
    $outputPath = Join-Path $outputDirPath $asset.Name
    $width = $asset.Width
    $height = $asset.Height
    ...
    $result = Invoke-Magick $magickArgs
    ...
    if ($result.Succeeded) {
        ...
        Write-Host "[FAIL] Failed to generate: $($asset.Name) - $($result.Output)" -ForegroundColor Red
        $failCount++
    }
}
