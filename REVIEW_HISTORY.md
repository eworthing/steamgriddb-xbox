...
--- Loop 14 (UTC 2026-08-04T18:16:40Z) ---

### Discovery
See Loop 1 Discovery (refreshed Loop 7). Ground truth this loop: 131 tests passing before and after; MSBuild
exit 0 both runs; zero source drift since loop 13's commit `6e83b10`. Selected lens: Generic (C#/.NET /
UWP-hosted WinUI stack).
...
- **Architecture quality:** 7.0 | SAME | Re-read `LoadGameEntriesAsync` (`PrimaryWidget.xaml.cs:332-611`)
  ...
  HEAD -- PrimaryWidget.xaml.cs` (empty) - unchanged for five loops running (9 through 14). Read the
  image-resolution block (`:480-538`) and the network-resolution block (`:551-629`) looking for a slice
  separable the way `ManifestEntryIdentity` was in loop 9. Neither qualifies: both blocks are I/O-heavy with no
  decision logic ahead of the fetch - extracting either into its own method would relocate I/O, not add
  testability or reduce coupling, so it fails Simplify Pressure Test Q2 - the same reasoning that has correctly
  kept `ArtworkDownloader.DownloadArtworkAsync` out of every extraction this run. This loop's actual fix
  targeted a different, smaller cluster (`StoreNameLookup.cs`) and does not touch this file - confirmed via
  `git diff --stat`.
- **State management and runtime ownership:** 7.0 | SAME | `AppliedArtworkStore.cs` and `FixLog.cs` production
  ...
  `StoreNameLookup`'s caches are shared static state; this run has consistently scored that class's caches under
  `data_flow` since the loop-3 file split, not `state_management`.
- **Domain modeling:** 5.5 | SAME | Not re-litigated this loop per this run's standing finding (loop 11
...
- **Data flow and dependency design:** 6.5 | UP | `StoreNameLookup.cs`'s `ubisoftGameLookupCache` field and
  ...
  single-value-loaded-once shape. Structural proof: `git diff -- SteamGridDB.Xbox/Services/Stores/
  StoreNameLookup.cs` (21 insertions, 23 deletions, net -2 lines). First loop in this run's history to land a
  source-level change against `data_flow`'s own named residual rather than holding it flat (unchanged loop 3
  through loop 13, eleven loops). Residual narrows, does not close: `gogNameCache`/`epicNameCache`/
  `nameMatchCache` remain unlocked per-key dictionaries - a different shape `AsyncLazyCache<T>` does not fit -
  and `EpicLibrary.cs`'s ambient `Environment.GetEnvironmentVariable("ProgramData")` fallback is untouched.
- **Framework / platform best practices:** 6.0 | SAME | Not re-litigated this loop per this run's standing
  finding (loop 11 SPT-rejected treating the `DataContractJsonSerializer` / `Windows.Data.Json` split in
  `SteamGridDbClient.cs` as a framework-idiom violation). Confirmed via `git diff --stat 85b5279 HEAD --
  SteamGridDbClient.cs` (empty) that no new evidence exists this loop.
- **Concurrency and runtime safety:** 6.5 | SAME | F-003's sequential per-game awaits unaffected - `git diff
  --stat` confirms `PrimaryWidget.xaml.cs` untouched this loop. This loop's fix adds a lock to a cache already
  safe under the current single-caller sequential access pattern - no behavior change, no new concurrent
  caller, does not touch F-003's blocked parallelization question. Scored entirely under `data_flow`.
- **Code simplicity and clarity:** 8.5 | SAME | This loop's diff (`StoreNameLookup.cs`, 21 insertions/23
  ...
  simplification - but per this run's anti-double-counting convention the credit is scored entirely under
  `data_flow`. No ceremony added; net line count dropped.
- **Test strategy and regression resistance:** 9.5 | SAME | Unaffected this loop - `git diff --stat` confirms
  ...
  still met. Loop 13's queued residual (`ArtworkSignatureTests.cs` + the `officialArtworkFloor` gate
  extraction) was deliberately deferred this loop in favor of the `data_flow` investigation the dispatch asked
  for - it remains Priority 1 in this loop's backlog.
- **Overall implementation credibility:** 7.5 | SAME | Consistent with this run's anti-double-counting
  ...
  methods (including the now-fixed `GetUbisoftGameNameAsync`) remain untested by direct test - network-bound,
  no seam - so this fix's correctness rests on the two green gates plus direct source inspection, not a new
  assertion.

## Authority Map

- **Concern:** Applied-artwork record. **Owner:** `AppliedArtworkStore`. **Verdict:** Single and clear. Direct
  test: `AppliedArtworkStoreTests.cs`. Unaffected this loop.
- **Concern:** Fix-run diagnostic log. **Owner:** `FixLog`. **Verdict:** Single and clear. Direct test:
  `FixLogTests.cs`. Unaffected this loop.
- **Concern:** Store-name lookup caches and the artwork download/selection gate. **Owner:** `StoreNameLookup`
  and `ArtworkDownloader`. **Verdict:** Single and clear ownership. One of four `StoreNameLookup` caches moved
  from unlocked to gated this loop (`ubisoftGameListCache`); three remain unlocked per-key dictionaries - a
  different shape, queued as backlog item 2. Test gap unchanged from loop 13: `ArtworkDownloaderTests.cs`
  directly tests the replacement gate's colour/layout decision; the three async entry points, `StoreNameLookup`'s
  network calls, and `ArtworkSignature.cs` remain untested (Priority 1, F-007).

## Strengths That Matter

- `ArtworkSource`'s private-constructor-plus-factory-method design still makes "neither a platform-ID nor a
  game-ID" unrepresentable - unaffected this loop.
- This loop closed a residual the standing scorecard text had named unchanged for eleven consecutive loops by
  finding a fourth instance of the same smell the standing text had never counted, and fixing it by pure reuse
  of an in-house type rather than inventing anything new.
- `AsyncLazyCache<T>`'s original design (taking the caller's own lock as a constructor argument) let this
  loop's fix reach a third call site with zero changes to the shared type itself and zero new test
  infrastructure - the abstraction earned its keep a second time since loop 6.
...
backed by a new `gate` `SemaphoreSlim`, preserving the original's do-not-cache-a-failed-or-empty-fetch
semantics.
...
when deleted, because `AsyncLazyCache<T>` already provides identical, already-tested behavior.

**Dependency category** - n/a

**Leverage impact** - One caller (`GetUbisoftGameNameAsync`); reduces the number of competing caching idioms
in this file from two to one.

**Locality impact** - Change scoped entirely to `StoreNameLookup.cs`.
...
inside one 317-line file work against Locality.
...
EpicLibrary's and AppliedArtworkStore's own idiom exactly. Done this loop.

**Blast radius** - Change: `SteamGridDB.Xbox/Services/Stores/StoreNameLookup.cs`. Avoid: `SteamGridDB.Xbox/
Services/Stores/EpicLibrary.cs`, `SteamGridDB.Xbox/Services/Artwork/AppliedArtworkStore.cs`,
`SteamGridDB.Xbox/Services/AsyncLazyCache.cs`, `SteamGridDB.Xbox/PrimaryWidget.xaml.cs`.
...
**Why it matters** - The largest file in the repo across every prior loop's Discovery continues to bundle
several structurally distinct concerns with no Module boundary between most of them.

**What is wrong** - Re-read `LoadGameEntriesAsync` directly this loop (`PrimaryWidget.xaml.cs:332-611`), the
first fresh re-investigation of this file's separability since loop 8. Read the image-resolution block
(`:480-538`) and the network-resolution block (`:551-629`) specifically looking for a slice separable the way
`ManifestEntryIdentity` was in loop 9. Neither qualifies: both blocks are I/O-heavy with no decision logic
ahead of the fetch to extract - the same shape that has correctly kept `ArtworkDownloader.DownloadArtworkAsync`
itself out of every extraction attempted this run. Fails Simplify Pressure Test Q2. This loop's own diff
touches only `StoreNameLookup.cs`; confirmed via `git diff --stat 05501e0 HEAD -- PrimaryWidget.xaml.cs`
(empty) that the file has been byte-identical for five loops running (9 through 14).
...
candidate extractions were rejected via Simplify Pressure Test Q2).
...
separable decision logic, re-confirmed by direct read this loop. Consistent with this run's discipline since
loop 8.
...
ship visibly bad artwork with no test catching it. Sole remaining Authority-Map test-coverage gap, unchanged
since loop 13 (deliberately deferred this loop).
...
`FindOfficialLookalikeAsync` still orchestrate that network call, as do `StoreNameLookup`'s four network-bound
writer methods and `ArtworkSignature`'s `ColourMatch`/`LayoutMatch`/`CreateAsync` (zero test file).

**Evidence**
- `SteamGridDB.Xbox/Services/Artwork/ArtworkDownloader.cs:35,40,71,122`
- `SteamGridDB.Xbox/Services/Stores/StoreNameLookup.cs`
- `SteamGridDB.Xbox/Services/Artwork/ArtworkSignature.cs`

**Architectural test failed** - Interface-as-test-surface

**Dependency category** - `true-external`

**Leverage impact** - One call site cluster (`ArtworkDownloader`'s gate).

**Locality impact** - Unaffected this loop.

**Metric signal** - Unchanged from loop 13.

**Why this weakens submission** - A source-level mutation in the download-loop's fallback-candidate capture,
loop boundaries, `StoreNameLookup`'s four writers, or `ArtworkSignature`'s computations would pass the entire
suite undetected today.
...
extract the `officialArtworkFloor` early-exit gate as a second small tested predicate.
...
| Field | Value |
|---|---|
| Structurally necessary | Replacing `StoreNameLookup`'s hand-rolled, unlocked `ubisoftGameLookupCache`/`LoadUbisoftGameListAsync` with the shared `AsyncLazyCache<T>` - passes the Deletion test. |
| New seam justified | false - `AsyncLazyCache<T>` is an existing, already-proven generic type reused at one more call site, not a new Seam. |
| Helpful simplification | `StoreNameLookup.cs` shrank 317 -> 315 lines (net -2) while removing one of two competing caching idioms in the file. |
| Should NOT be done | Attempting the same fix on `gogNameCache`/`epicNameCache`/`nameMatchCache` this loop - different shape, needs its own SPT pass (queued). Also not attempted: `LoadGameEntriesAsync`'s further split (still fails SPT), forcing a `domain_modeling`/`framework_idioms` finding, or building `ArtworkSignatureTests.cs` (deferred to next loop). |
| Tests after fix | No test file exists for `StoreNameLookup.cs`'s network-bound methods before or after - network-bound, no seam. `AsyncLazyCache<T>` itself is unchanged and already covered by `AsyncLazyCacheTests.cs`. |

## Improvement Backlog

1. Add `ArtworkSignatureTests.cs` and extract the `officialArtworkFloor` gate as a second tested predicate
   (F-007, carried forward from loop 13, deliberately deferred this loop).
2. Extend F-009's fix to `StoreNameLookup`'s three remaining unlocked per-key caches
   (`gogNameCache`/`epicNameCache`/`nameMatchCache`) - a bigger, riskier change; re-run Simplify Pressure Test
...
- **Candidate Module:** `StoreNameLookup`'s three remaining per-key caches. Friction proven Finding F-009: the
  file's fourth cache duplicated an existing, already-tested abstraction and was fixed by pure reuse; the other
  three share the same smell but a different shape (per-key, not single-value). `in-process`. Smallest first
  step: wrap `gogNameCache`'s access in `GetOrFetchGogNameAsync` with the new `gate` first, prove the pattern,
  then repeat for the other two. What not to do: do not introduce `ConcurrentDictionary` as a drop-in
  replacement - it does not make the check-then-act sequence atomic; the `gate`-around-every-access idiom
  `AppliedArtworkStore` already uses is the proven answer.

## Builder Notes

1. **Pattern:** A scorecard residual can sit unchanged for many loops not because it is unfixable, but because
   every loop re-confirms the SAME evidence without asking whether a DIFFERENT, previously-uncounted instance
   of the same smell exists nearby. → REVIEW_HISTORY.json `loops[14].builder_notes` for full notes.
2. **Pattern:** When a codebase already has a generic, tested type for a shape, a hand-rolled duplicate of that
   exact shape elsewhere is a Deletion-test pass waiting to be noticed. → REVIEW_HISTORY.json
   `loops[14].builder_notes` for full notes.
3. **Pattern:** Adding a brand-new, self-contained lock where none existed before is a smaller risk than moving
   or restructuring an existing one, but it is still worth recording as risk-boundary evidence. →
   REVIEW_HISTORY.json `loops[14].builder_notes` for full notes.

## Final Judge Narrative

Place, not win. Ground truth was clean going in and clean coming out (131/131 tests, MSBuild exit 0). This
loop's dispatch named a real pattern - eight of the last nine loops moved only `test_strategy`/`simplicity` -
and asked for a genuine investigation of the structural dimensions before defaulting to the queued
test-coverage item. Both `architecture_quality` and `data_flow` got that investigation. `architecture_quality`
held, for a documented reason: the two candidate blocks inside `LoadGameEntriesAsync` are both I/O-only with no
decision logic ahead of the fetch, the same shape that has correctly protected
`ArtworkDownloader.DownloadArtworkAsync` from extraction all run. `data_flow` did not hold: a fourth static
cache in `StoreNameLookup.cs`, never counted in the standing "three unlocked caches" text, hand-rolled an
unlocked duplicate of the codebase's own `AsyncLazyCache<T>`. Fixed by pure reuse - no new type, no new Seam,
net -2 lines - closing the first source-level gap this run's `data_flow` scorecard has closed in eleven loops.
The score moves 6.0 to 6.5 on that structural proof; it does not reach the 7-anchor, since three more caches
and one ambient environment-variable fallback remain untouched, honestly queued rather than folded into this
loop's smaller, more defensible fix. Runtime ownership and concurrency are unaffected and exactly as
trustworthy as loop 13 left them - this loop's diff touches one production file, adds one new self-contained
lock whose critical section contains no other lock (recorded as risk-boundary evidence), and `git diff --stat`
confirms every other production file byte-identical since loop 13's commit. Tests neither improved nor
regressed this loop - the fixed method remains outside the test suite's network boundary by design. Future
work risks nothing new from overengineering - this loop's fix added zero new abstraction and explicitly
declined to extend the same fix to three riskier, differently-shaped caches without a fresh SPT pass. Backlog
is not empty (loop 13's queued test-coverage item returns to Priority 1; the new cache-locking slice and F-003
follow), so `CONTINUE`.
...
lock is acquired within that scope, so no lock-ordering hazard is introduced. AsyncLazyCache<T> itself is
unchanged and already stress-tested under 32 concurrent callers (AsyncLazyCacheTests.cs, pre-existing). No
thread-sanitizer or concurrency stress-test tooling exists for this C#/UWP stack in this environment; verified
instead by direct inspection of the one new lock's scope, and independently confirmed by the Step-3
implementation reviewer.", "mechanically_testable": false}`

**Targeted finding status:** `resolved` - F-009's Claim is fully gone from current source; the old field and
method no longer exist, replaced entirely by a call into the existing shared type.

**Unintended scorecard regression:** none. Eight of nine dimensions held SAME with fresh structural
re-derivation this loop; `data_flow` moved UP on structural proof. No dimension regressed.
...
--- Loop 15 (UTC 2026-08-04T18:47:32Z) ---
...
| Field | Value |
|---|---|
| Structurally necessary | Extracting `ArtworkDownloader.ChosenAlreadyMatchesOfficialArt` from the inline `chosenMatch >= officialArtworkFloor` check — mirrors loop 13's `PassesColourAndLayoutGate` extraction exactly. |
| New seam justified | false — no new Seam; a pure predicate extraction and new test file only. |
| Helpful simplification | None claimed — purely additive work, credited entirely to `test_strategy`. |
| Should NOT be done | Building F-010's queued fix (locking the three remaining per-key caches) this loop on loop 14's precedent alone — fresh SPT found the literal plan does not pass. Also not attempted: further splitting `LoadGameEntriesAsync`, or re-litigating `domain_modeling`/`framework_idioms` without new evidence. |
| Tests after fix | New `ArtworkSignatureTests.cs` (5 tests) at `ArtworkSignature`'s real public Interface; 2 new boundary tests in `ArtworkDownloaderTests.cs` mirroring `PassesColourAndLayoutGate`'s shape. No old tests to delete (zero coverage existed before). |
...
1. A backlog item queued as "extend fix X to the remaining N instances" can look like a drop-in repeat of
   the fix that just worked, when the remaining instances are actually a different shape that the same
   remedy does not fit. → REVIEW_HISTORY.json `loops[14].builder_notes` for full notes.
2. A single shared lock protecting several logically-independent resources is invisible as a cost when
   there is only ever one caller at a time, and becomes a real, silent bottleneck the moment concurrency is
   introduced. → REVIEW_HISTORY.json `loops[14].builder_notes` for full notes.
3. Mutation testing a brand-new test file can surface a real gap even inside the code the same loop just
   wrote — "I added tests for X" is not the same claim as "every mutation of X is now caught." →
   REVIEW_HISTORY.json `loops[14].builder_notes` for full notes.
